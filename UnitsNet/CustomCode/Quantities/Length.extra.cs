...
namespace UnitsNet
{
    public partial struct Length
    {
        ...
        ///     Optionally specify the culture format numbers and localize unit abbreviations. Defaults to
        ///     thread's culture.
        /// </param>
        ...
        public static Length ParseFeetInches([NotNull] string str, IFormatProvider formatProvider = null)
        {
            ...
            if (!TryParseFeetInches(str, out var result, formatProvider))
            {
        ...
        public static bool TryParseFeetInches([CanBeNull] string str, out Length result, IFormatProvider formatProvider = null)
        {
            ...
            var footRegex = quantityParser.CreateRegexPatternForUnit(LengthUnit.Foot, formatProvider, false);
            var inchRegex = quantityParser.CreateRegexPatternForUnit(LengthUnit.Inch, formatProvider, false);

            // Match entire string exactly
            var pattern = $@"^(?<negativeSign>\-?)(?<feet>{footRegex})\s?(?<inches>{inchRegex})$";
            ...
            if (TryParse(feetGroup.Value, formatProvider, out var feet) &&
                TryParse(inchesGroup.Value, formatProvider, out var inches))
            {
                result = feet + inches;
