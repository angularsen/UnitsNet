...
using UnitsNet;
using UnitsNet.Modular.Playground;
...
Length[] routeLegs = routeInputs
    .Select(text => Length.Parse(text, culture))
    .ToArray();

Length depotDetour = Length.TryParse("12 km", culture, out Length parsedDetour)
    ? parsedDetour
    : Length.Zero;
...
string mileAbbreviation = Length.GetAbbreviation(LengthUnit.Mile, culture);

Console.WriteLine($"Route legs: {string.Join(", ", routeLegs.Select(FormatStoredValue))}");
Console.WriteLine(
    $"Total:      {totalRoute.ToString("0.0", culture)} / " +
    $"{totalRouteMiles.ToString("0.0", culture)} {mileAbbreviation}");
Console.WriteLine($"Average:    {averageLeg.ToString("0.0", culture)} per scheduled leg");

double localSpeedLimitMph = Speed.Convert(
    80,
    SpeedUnit.KilometerPerHour,
    SpeedUnit.MilePerHour);
Console.WriteLine($"80 km/h local limit = {localSpeedLimitMph.ToString("F1", culture)} mph");
...
Duration drivingTime = Duration.Parse("4 h", culture) + Duration.FromMinutes(25);
Speed averageSpeed = totalRoute / drivingTime;
Speed displaySpeed = averageSpeed.ToUnit(SpeedUnit.KilometerPerHour);

Console.WriteLine($"Driving time:  {drivingTime.ToString("0.0", culture)}");
Console.WriteLine($"Average speed: {displaySpeed.ToString("0.0", culture)}");
...
Console.WriteLine($"Charger:       {charger.ToString("0", culture)}");
Console.WriteLine($"Energy in 22m: {displayEnergy.ToString("0.0", culture)}");
Console.WriteLine($"Time for 42kWh: {displayChargeTime.ToString("0.0", culture)}");
...
ParcelCount manifest = ParcelCount.Parse("2 doz", culture) + ParcelCount.FromParcels(6);
...
Console.WriteLine(
    $"Manifest: {parcelManifest.ToString("0", culture)} " +
    $"({dozenManifest.ToString("0.00", culture)})");
...
string formattedConfiguredDistance = lengthDescriptor.Format(configuredDistance, "0.0", culture);
Console.WriteLine(
    $"Configured leg: {formattedConfiguredDistance} = " +
    $"{configuredKilometers.ToString("F1", culture)} km");
...
Console.WriteLine($"SI policy:     {siRoute.ToString("0", culture)}");
...
Console.WriteLine($"Round trip:{restoredEnergy.ToString("0.0", culture),10}");

Console.WriteLine();
Console.WriteLine("Try editing ApplicationUnits.cs, ParcelCount.unitsnet.json, or this file, then run:");
Console.WriteLine("  dotnet run");

string FormatStoredValue(Length length) => length.ToString("0.#", culture);
