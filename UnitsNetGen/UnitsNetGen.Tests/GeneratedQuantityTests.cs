...
public sealed class GeneratedQuantityTests
{
    ...
    public void GeneratedRegistry_SystemTextJsonRoundTripsBuiltInAndCustomQuantities()
    {
        ...
        options.Converters.Add(global::UnitsNetGen.Generated.UnitsNetGenRegistry.JsonConverter);
    ...
    {
        UnitsNetGen.QuantityRegistry registry = global::UnitsNetGen.Generated.UnitsNetGenRegistry.Instance;
    ...
    public void GeneratedRegistry_FindsOnlySelectedQuantitiesWithMatchingDimensions()
    {
        ...
        IReadOnlyList<UnitsNetGen.IQuantityDescriptor> matches =
        ...
        Assert.Throws<NotSupportedException>(
            () => ((IList<UnitsNetGen.IQuantityDescriptor>)matches).Add(registry.Get("Area")));
    }
    ...
    public void GeneratedQuantityFacade_ProvidesOwnerScopedDynamicApi()
    {
        ...
        Assert.Same(global::UnitsNetGen.Generated.UnitsNetGenRegistry.Instance, Quantity.Registry);
        ...
        Assert.IsType<Length>(created);
        Assert.True(Quantity.TryFrom(2, "Length", UnitsNetGen.UnitSystem.SI, out created));
        Assert.IsType<Length>(created);
    ...
    public void DynamicQuantityContracts_DoNotExposeObjectValues()
    {
        Type[] contractTypes =
        {
            typeof(UnitsNetGen.IQuantityDescriptor),
            typeof(UnitsNetGen.QuantityDescriptor<Length, LengthUnit>),
            typeof(UnitsNetGen.QuantityRegistry),
            typeof(Quantity),
        };
    ...
    public void GeneratedRegistry_RetainsBaseUnitsAndResolvesImmutableUnitSystems()
    {
        ...
        UnitsNetGen.IQuantityDescriptor force = registry.Get("Force");
        UnitsNetGen.IQuantityDescriptor density = registry.Get("Density");
        UnitsNetGen.IQuantityDescriptor information = registry.Get("Information");

        Assert.Equal(
            new UnitsNetGen.BaseUnits(length: "Meter"),
            length.Units.Single(unit => unit.Name == "Meter").BaseUnits);
        Assert.Equal(
            new UnitsNetGen.BaseUnits(length: "Kilometer"),
            length.Units.Single(unit => unit.Name == "Kilometer").BaseUnits);
        Assert.Equal(
            new UnitsNetGen.BaseUnits(length: "Meter", mass: "Kilogram", time: "Second"),
            force.Units.Single(unit => unit.Name == "Newton").BaseUnits);
        Assert.Equal(
            new UnitsNetGen.BaseUnits(length: "Meter", mass: "Kilogram"),
            density.Units.Single(unit => unit.Name == "KilogramPerCubicMeter").BaseUnits);

        Assert.Equal("Meter", length.GetUnit(UnitsNetGen.UnitSystem.SI).Name);
        Assert.Equal("Newton", force.GetUnit(UnitsNetGen.UnitSystem.SI).Name);
        Assert.Equal("KilogramPerCubicMeter", density.GetUnit(UnitsNetGen.UnitSystem.SI).Name);
        Assert.Equal(information.BaseUnitName, information.GetUnit(UnitsNetGen.UnitSystem.SI).Name);

        var imperialLength = new UnitsNetGen.UnitSystem(
            new UnitsNetGen.BaseUnits(length: "Foot"));
        Assert.Equal("Foot", length.GetUnit(imperialLength).Name);
        Assert.True(length.TryGetUnit(imperialLength, out UnitsNetGen.UnitDescriptor? selected));
        Assert.Equal("Foot", selected!.Name);
        Assert.Equal(LengthUnit.Meter, new Length(2, UnitsNetGen.UnitSystem.SI).Unit);
        Assert.Equal(LengthUnit.Meter, Length.From(2, UnitsNetGen.UnitSystem.SI).Unit);
        Assert.Equal(ForceUnit.Newton, new Force(2, UnitsNetGen.UnitSystem.SI).Unit);
        Assert.Equal(Information.BaseUnit, new Information(2, UnitsNetGen.UnitSystem.SI).Unit);
        Assert.Equal(LengthUnit.Foot, new Length(2, imperialLength).Unit);
        Assert.Equal(1000, Length.FromKilometers(1).As(UnitsNetGen.UnitSystem.SI), 10);
        Assert.Equal(LengthUnit.Meter, Length.FromKilometers(1).ToUnit(UnitsNetGen.UnitSystem.SI).Unit);

        var registryValue = Assert.IsType<Length>(
            registry.Create(2, "Length", UnitsNetGen.UnitSystem.SI));
        Assert.Equal(LengthUnit.Meter, registryValue.Unit);
        Assert.Equal(
            1000,
            registry.Convert(1, "Length", "Kilometer", UnitsNetGen.UnitSystem.SI),
            10);
        Assert.True(
            registry.TryCreate(
                2,
                "Length",
                UnitsNetGen.UnitSystem.SI,
                out UnitsNet.Core.IQuantity<double>? created));
        Assert.IsType<Length>(created);
        Assert.True(
            registry.TryConvert(
        ...
        var unavailable = new UnitsNetGen.UnitSystem(
            new UnitsNetGen.BaseUnits(length: "Smoot"));
        ...
        Assert.Throws<ArgumentException>(() => new UnitsNetGen.UnitSystem(UnitsNetGen.BaseUnits.Undefined));
        Assert.True(
            new UnitsNetGen.BaseUnits(length: "Meter", time: "Second")
                .IsSubsetOf(UnitsNetGen.UnitSystem.SI.BaseUnits));
        Assert.False(
            new UnitsNetGen.BaseUnits(length: "Foot")
                .IsSubsetOf(UnitsNetGen.UnitSystem.SI.BaseUnits));
    }
