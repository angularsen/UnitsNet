# UnitsNet.Modular prototype implementation plan
...
- UnitsNet v6 adoption of `UnitsNet.Core`: `agl/unitsnetgen-poc-unitsnet-integration`, stacked on
  `agl/unitsnetgen-poc`
- Steps 6–7: `agl/unitsnetgen-poc-numeric-type`, created from the completed step-5 commit
...
- [x] **1. Add linked-source compatibility consumers**
  - Add separate UnitsNet and UnitsNet.Modular console projects.
...
- [x] **4. Introduce shared abstractions/Core**
  ...
  - Have UnitsNet.Modular quantities implement the shared contract.
...
- [x] **Follow-up: replace hardcoded relationships with catalog data**
  ...
  - Accept custom `UnitsNetRelation` files through compiler `AdditionalFiles`.
...
- [x] **8. Establish consumer-owned generation and definition packages**
  - Put stable authoring attributes and selection interfaces in the UnitsNet.Modular runtime instead of
...
- [x] **13. Share quantity capability contracts and algorithms**
  ...
  - Keep instance `As`/`ToUnit` APIs while exposing static conversion for generic algorithms without
    a global lookup.
  - Reuse `Sum` and `Average` across UnitsNet and UnitsNet.Modular linear quantities.
...
- [x] **15. Establish comprehensive compatibility gates**
  ...
  - Compile a shared concrete-consumer source corpus against UnitsNet and UnitsNet.Modular.
