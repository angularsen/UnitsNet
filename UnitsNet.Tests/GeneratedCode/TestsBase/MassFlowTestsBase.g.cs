...
// ReSharper restore VirtualMemberNeverOverriden.Global
        ...
        public void ToString_WithSwedishCulture_ReturnsUnitAbbreviationForEnglishCultureSinceThereAreNoMappings()
        {
            ...
            Assert.Equal("1 cg/d", new MassFlow(1, MassFlowUnit.CentigramPerDay).ToString(null, swedishCulture));
            Assert.Equal("1 cg/s", new MassFlow(1, MassFlowUnit.CentigramPerSecond).ToString(null, swedishCulture));
            Assert.Equal("1 dag/d", new MassFlow(1, MassFlowUnit.DecagramPerDay).ToString(null, swedishCulture));
            Assert.Equal("1 dag/s", new MassFlow(1, MassFlowUnit.DecagramPerSecond).ToString(null, swedishCulture));
            Assert.Equal("1 dg/d", new MassFlow(1, MassFlowUnit.DecigramPerDay).ToString(null, swedishCulture));
            Assert.Equal("1 dg/s", new MassFlow(1, MassFlowUnit.DecigramPerSecond).ToString(null, swedishCulture));
            Assert.Equal("1 g/d", new MassFlow(1, MassFlowUnit.GramPerDay).ToString(null, swedishCulture));
            Assert.Equal("1 g/h", new MassFlow(1, MassFlowUnit.GramPerHour).ToString(null, swedishCulture));
            Assert.Equal("1 g/s", new MassFlow(1, MassFlowUnit.GramPerSecond).ToString(null, swedishCulture));
            Assert.Equal("1 hg/d", new MassFlow(1, MassFlowUnit.HectogramPerDay).ToString(null, swedishCulture));
            Assert.Equal("1 hg/s", new MassFlow(1, MassFlowUnit.HectogramPerSecond).ToString(null, swedishCulture));
            Assert.Equal("1 kg/d", new MassFlow(1, MassFlowUnit.KilogramPerDay).ToString(null, swedishCulture));
            Assert.Equal("1 kg/h", new MassFlow(1, MassFlowUnit.KilogramPerHour).ToString(null, swedishCulture));
            Assert.Equal("1 kg/min", new MassFlow(1, MassFlowUnit.KilogramPerMinute).ToString(null, swedishCulture));
            Assert.Equal("1 kg/s", new MassFlow(1, MassFlowUnit.KilogramPerSecond).ToString(null, swedishCulture));
            Assert.Equal("1 Mg/d", new MassFlow(1, MassFlowUnit.MegagramPerDay).ToString(null, swedishCulture));
            Assert.Equal("1 Mlb/d", new MassFlow(1, MassFlowUnit.MegapoundPerDay).ToString(null, swedishCulture));
            Assert.Equal("1 Mlb/h", new MassFlow(1, MassFlowUnit.MegapoundPerHour).ToString(null, swedishCulture));
            Assert.Equal("1 Mlb/min", new MassFlow(1, MassFlowUnit.MegapoundPerMinute).ToString(null, swedishCulture));
            Assert.Equal("1 Mlb/s", new MassFlow(1, MassFlowUnit.MegapoundPerSecond).ToString(null, swedishCulture));
            Assert.Equal("1 µg/d", new MassFlow(1, MassFlowUnit.MicrogramPerDay).ToString(null, swedishCulture));
            Assert.Equal("1 µg/s", new MassFlow(1, MassFlowUnit.MicrogramPerSecond).ToString(null, swedishCulture));
            Assert.Equal("1 mg/d", new MassFlow(1, MassFlowUnit.MilligramPerDay).ToString(null, swedishCulture));
            Assert.Equal("1 mg/s", new MassFlow(1, MassFlowUnit.MilligramPerSecond).ToString(null, swedishCulture));
            Assert.Equal("1 ng/d", new MassFlow(1, MassFlowUnit.NanogramPerDay).ToString(null, swedishCulture));
            Assert.Equal("1 ng/s", new MassFlow(1, MassFlowUnit.NanogramPerSecond).ToString(null, swedishCulture));
            Assert.Equal("1 lb/d", new MassFlow(1, MassFlowUnit.PoundPerDay).ToString(null, swedishCulture));
            Assert.Equal("1 lb/h", new MassFlow(1, MassFlowUnit.PoundPerHour).ToString(null, swedishCulture));
            Assert.Equal("1 lb/min", new MassFlow(1, MassFlowUnit.PoundPerMinute).ToString(null, swedishCulture));
            Assert.Equal("1 lb/s", new MassFlow(1, MassFlowUnit.PoundPerSecond).ToString(null, swedishCulture));
            Assert.Equal("1 short tn/h", new MassFlow(1, MassFlowUnit.ShortTonPerHour).ToString(null, swedishCulture));
            Assert.Equal("1 t/d", new MassFlow(1, MassFlowUnit.TonnePerDay).ToString(null, swedishCulture));
            Assert.Equal("1 t/h", new MassFlow(1, MassFlowUnit.TonnePerHour).ToString(null, swedishCulture));
        }
