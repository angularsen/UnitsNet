...
code-generation model. UnitsNetGen uses the small `UnitsNet.Core` contract assembly. The contracts
...
Package assets under `buildTransitive/` retain `UnitsNetGenDefinition` and `UnitsNetGenRelation` as
...
UnitsNetGen instead provides immutable, owner-neutral `UnitSystem` and `BaseUnits` values that are
...
The `UnitsNetGen` runtime and `UnitsNet.Core` supply assets for .NET 8, 9, and 10. The standalone
...
`AdditionalFiles` items marked with `UnitsNetGenRelation="true"`:
