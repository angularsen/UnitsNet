...
namespace UnitsNet.Serialization.JsonNet.Tests
{
    public sealed class UnitsNetBaseJsonConverterTest
    {
        ...
        private readonly Power unit;

        private TestConverter sut;

        public UnitsNetBaseJsonConverterTest()
        {
            sut = new TestConverter();
            unitName = "PowerUnit.Watt";
            unitValue = 10.2365D;
            unit = Power.FromWatts(unitValue);
        }

        [Fact]
        public void UnitsNetBaseJsonConverter_ConvertIQuantity_works_as_expected()
        {
            var result = sut.Test_ConvertIQuantity(unit);

            Assert.Equal(unitName, result.Unit);
            Assert.Equal(unitValue, result.Value);
        }

        [Fact]
        public void UnitsNetBaseJsonConverter_ConvertIQuantity_throws_ArgumentNullException_when_quantity_is_NULL()
        {
            var result = Assert.Throws<ArgumentNullException>(() => sut.Test_ConvertIQuantity(null));
        ...
        {
            var result = sut.Test_ConvertValueUnit(unitName, unitValue);

            Assert.NotNull(result);
            Assert.IsType<Power>(result);
            Assert.True(unit.Equals((Power)result, 1E-5, ComparisonType.Absolute));
        ...
        {
            var result = sut.Test_ConvertValueUnit();
        ...
        {
            var result = Assert.Throws<UnitsNetException>(() => sut.Test_ConvertValueUnit("SomeImaginaryUnit.Watt", unitValue));
        ...
        {
            var result = Assert.Throws<UnitsNetException>(() => sut.Test_ConvertValueUnit("PowerUnit Watt", unitValue));
        ...
        public void UnitsNetBaseJsonConverter_CreateLocalSerializer_works_as_expected()
        {
            ...
            var result = sut.Test_CreateLocalSerializer(serializer);
        ...
        public void UnitsNetBaseJsonConverter_ReadValueUnit_work_as_expected()
        {
            ...
            token.Add("Unit", unitName);
            token.Add("Value", unitValue);

            var result = sut.Test_ReadValueUnit(token);
        ...
        public void UnitsNetBaseJsonConverter_ReadValueUnit_works_case_insensitive(string unitPropertyName, string valuePropertyName)
        {
            ...
            token.Add(unitPropertyName, unitName);
            token.Add(valuePropertyName, unitValue);
