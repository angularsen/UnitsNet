...
internal static class QuantityRelationPlanner
{
        ...
        out IReadOnlyList<string> errors)
    {
        ...
        foreach (QuantityRelation relation in relations)
        {
            ...
            if (relation.Result.Quantity is not ("1" or "double") &&
                !TryResolve(
                    relation.Result,
                    bySemanticId,
                    byName,
            ...
            bool isSupportedShape =
                ...
                left is not null &&
                right is not null &&
                relation.Operator is "*" or "/" or "inverse" ||
                result is not null &&
                left is not null &&
                right is not null ||
                result is not null &&
            ...
            if (!isSupportedShape)
            {
                planningErrors.Add($"Relation '{relation.Source}' has an unsupported scalar shape.");
                continue;
            }
        ...
        foreach (IGrouping<string, EmittedQuantityRelation> inverseMembers in applicable
                     .Where(relation => relation.Operator == "inverse" && relation.Left is not null)
        ...
        {
            string ownerKey = TargetKey((relation.Left ?? relation.Right)!.Definition);
    ...
    private static string OperatorSignature(EmittedQuantityRelation relation) =>
        (relation.Left?.Definition.SemanticId ?? "double") + " " + relation.Operator + " " +
        (relation.Right?.Definition.SemanticId ?? "double");
