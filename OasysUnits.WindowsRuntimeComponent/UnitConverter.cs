...
namespace OasysUnits
{
    ...
    public static class UnitConverter
    {
        ...
        private static readonly Assembly OasysUnitsAssembly = typeof(Length).Wrap().Assembly;

        private static readonly Type[] QuantityTypes = OasysUnitsAssembly.GetTypes()
        ...
        private static readonly Type[] UnitTypes = OasysUnitsAssembly.GetTypes()
        ...
        public static double ConvertByAbbreviation(double fromValue, string quantityName, string fromUnitAbbrev, string toUnitAbbrev, string culture)
        {
            ...
            var fromMethod = GetStaticFromMethod(quantityType, unitType); // ex: OasysUnits.Length.From(double inputValue, LengthUnit inputUnit)
            var fromResult = fromMethod.Invoke(null, new object[] {fromValue, fromUnitValue}); // ex: Length quantity = OasysUnits.Length.From(5, LengthUnit.Meter)
        ...
        {
            // Want to match: Length l = OasysUnits.Length.From(double inputValue, LengthUnit inputUnit)
            // Do NOT match : Length? OasysUnits.Length.From(double? inputValue, LengthUnit inputUnit)
            return quantityType.Wrap().GetDeclaredMethods()
