...
[Generator]
public sealed class UnitsNetModularGenerator : IIncrementalGenerator
{
    ...
    private static readonly DiagnosticDescriptor MultipleModules = new DiagnosticDescriptor(
        "UNM014",
        "Multiple UnitsNet.Modular modules are not supported",
        ...
        ImmutableArray<RelationDefinitionResult> relationDefinitionResults)
    {
        ...
        PrefixExpander.BaseUnitPrefixCatalog baseUnitPrefixes =
        ...
        foreach (QuantitySelection selection in selections)
        {
            ...
            context.AddSource(
                hintName,
                SourceText.From(
                    QuantityEmitter.Emit(selection, relationships, affineOffset, selectionsBySemanticId),
                    System.Text.Encoding.UTF8));

            foreach (CompanionTypeDefinition companion in BuiltInCompanionCatalog.Get(selection.Definition.SemanticId))
            {
                if (!BuiltInCompanionEmitter.CanEmit(companion, selection, selectionsBySemanticId))
                {
                    continue;
                }

                string companionHintName = modulePrefix + "_" +
                                           selection.Definition.TargetNamespace.Replace('.', '_') + "_" +
                                           companion.Kind + ".g.cs";
                context.AddSource(
                    companionHintName,
                    SourceText.From(
                        BuiltInCompanionEmitter.Emit(selection, companion),
                        System.Text.Encoding.UTF8));
            }
        }

        context.AddSource(
            modulePrefix + "_Registry.g.cs",
            SourceText.From(ModuleEmitter.EmitRegistry(selections), System.Text.Encoding.UTF8));
        context.AddSource(
            modulePrefix + "_Quantity.g.cs",
            SourceText.From(
                ModuleEmitter.EmitQuantityFacade(module.FacadeNamespace),
                System.Text.Encoding.UTF8));
        context.AddSource(
            modulePrefix + "_Extensions.g.cs",
            SourceText.From(ModuleEmitter.EmitExtensions(selections), System.Text.Encoding.UTF8));
    }
    ...
    private static bool IsJsonDefinition(AdditionalText file, AnalyzerConfigOptionsProvider optionsProvider)
    {
        ...
        return optionsProvider.GetOptions(file).TryGetValue(
                   "build_metadata.AdditionalFiles.UnitsNetDefinition",
                   out string? value) &&
    ...
    private static bool IsRelationDefinition(AdditionalText file, AnalyzerConfigOptionsProvider optionsProvider)
    {
        ...
        return optionsProvider.GetOptions(file).TryGetValue(
                   "build_metadata.AdditionalFiles.UnitsNetRelation",
                   out string? value) &&
               string.Equals(value, "true", StringComparison.OrdinalIgnoreCase);
    }
