...
[![UnitsNetGen CI](https://github.com/angularsen/UnitsNet/actions/workflows/unitsnetgen-ci.yml/badge.svg)](https://github.com/angularsen/UnitsNet/actions/workflows/unitsnetgen-ci.yml)

Generate only the strongly typed quantities and units your application needs.

UnitsNetGen combines a small runtime with a Roslyn source generator. A consumer selects quantities
...
> **Experimental:** UnitsNetGen is an alpha proof of concept. Its API, package structure, and
...
UnitsNetGen supports .NET 8, .NET 9, and .NET 10. Install the prerelease package in the project that
will own the generated quantities:

```shell
dotnet add package UnitsNetGen --prerelease
```
...
[UnitsNetModule("MyApplication.Units")]
...
Build the project. UnitsNetGen generates `Length`, `Duration`, `Speed`, their unit enums, and the
relationships between them into `MyApplication.Units`.
...
└── src
    ├── MyApplication.Units
    │   ├── MyApplication.Units.csproj -> UnitsNetGen + definition packages
    │   └── ApplicationUnits.cs
...
```mermaid
flowchart LR
    BuiltIns["UnitsNetGen built-in recipes"]
...
Selecting an affine quantity without its offset quantity produces diagnostic `UNG015`.
...
```csharp
Length length = Length.From(1.5, UnitsNetGen.UnitSystem.SI);
double meters = Length.FromKilometers(1.5).As(UnitsNetGen.UnitSystem.SI);
Length normalized = length.ToUnit(UnitsNetGen.UnitSystem.SI);
```
...
declaring multiple modules. `UNG014` reports multiple module markers before they emit colliding
types.

Without a target namespace, each definition keeps its declared namespace:

- built-in definitions generate into `UnitsNetGen`;
- custom definitions generate into their JSON `Namespace`;
- the generated `Quantity` facade is placed in the module interface's namespace.

Pass a target namespace to emit every selected quantity into one namespace:
...
The special target namespace `UnitsNet` emits quantity types in `UnitsNet` and unit enums in
`UnitsNet.Units` for source compatibility.
...
```csharp
IInclude<UnitsNetGen.BuiltIns.Length, MetricLengthUnits>
```

Built-in marker names match the quantity definition names in the UnitsNet catalog. The markers are
generated by the analyzer in `UnitsNetGen.BuiltIns`.

### Use profiles

UnitsNetGen currently supplies two profiles:

| Profile | Selection |
|---|---|
| `UnitsNetGen.Profiles.AllQuantities` | Every built-in catalog quantity and unit |
| `UnitsNetGen.Profiles.AllSi` | Focused SI mechanics chain used by the POC sample |
...
[UnitsNetModule]
internal interface ApplicationUnits :
    IIncludeProfile<UnitsNetGen.Profiles.AllQuantities>,
...
internal interface MechanicsProfile :
    IInclude<UnitsNetGen.BuiltIns.Length>,
    IInclude<UnitsNetGen.BuiltIns.Duration>,
    IInclude<UnitsNetGen.BuiltIns.Speed>;
...
```xml
<ItemGroup>
  <UnitsNetGenDefinition Include="Definitions/Length.json" />
  <UnitsNetGenRelation Include="Definitions/Relations.json" />
</ItemGroup>
```
...
| `Namespace` | No | Definition namespace and part of its semantic ID; defaults to `UnitsNetGen` |
...
UnitsNetGen validates and emits the expressions at compile time; it does not compile strings or use
...
UnitsNetGen.QuantityRegistry registry = UnitsNetGenRegistry.Instance;

UnitsNetGen.IQuantityDescriptor length = registry.Get("Length");
...
The generator also emits a `Quantity` facade into the module owner namespace:

```csharp
UnitsNet.Core.IQuantity<double> value =
    MyApplication.Units.Quantity.From(1.5, "Length", "Kilometer");

UnitsNet.Core.IQuantity<double> parsed =
    MyApplication.Units.Quantity.Parse(typeof(Length), "1.5 km");
```
...
UnitsNetGen reports authoring problems at compile time:

| ID | Meaning |
|---|---|
| `UNG001` | A selected marker has no built-in or JSON definition |
| `UNG002` | A unit pattern matched no unit |
| `UNG003` | A definition does not contain its declared base unit |
| `UNG004` | A JSON quantity definition is invalid |
| `UNG005` | Multiple JSON files provide the same semantic quantity ID |
| `UNG006` | A glob or regular expression is invalid |
| `UNG010` | A relation file is invalid |
| `UNG011` | Selected relations are ambiguous or cannot be resolved |
| `UNG012` | A selected unit-set marker has no patterns |
| `UNG013` | Definitions collide after applying the target namespace |
| `UNG014` | A compilation declares more than one module |
| `UNG015` | An affine quantity's offset quantity is not selected |

## Current scope and limitations

- UnitsNetGen is a design probe, not yet a committed replacement for UnitsNet.
...
- [Samples](https://github.com/angularsen/UnitsNet/tree/master/UnitsNetGen/Samples)
- [Consumer-owned package and project-reference scenarios](https://github.com/angularsen/UnitsNet/tree/master/UnitsNetGen/Samples/ConsumerOwned)
- [Custom definition package](https://github.com/angularsen/UnitsNet/tree/master/UnitsNetGen/Samples/DefinitionPackages/Fictional.Measurements.Definitions)
- [Architecture](https://github.com/angularsen/UnitsNet/blob/master/UnitsNetGen/ARCHITECTURE.md)
- [Migration notes](https://github.com/angularsen/UnitsNet/blob/master/UnitsNetGen/MIGRATION.md)
- [Implementation progress](https://github.com/angularsen/UnitsNet/blob/master/UnitsNetGen/IMPLEMENTATION_PLAN.md)
