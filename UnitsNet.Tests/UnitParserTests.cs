...
public class UnitParserTests
{
    ...
    {
        Assert.Throws<ArgumentNullException>(() => UnitParser.Default.GetUnitFromAbbreviation(null!, CultureInfo.InvariantCulture));
    }
    ...
    {
        Assert.Throws<UnitNotFoundException>(() => UnitParser.Default.GetUnitFromAbbreviation(unknownAbbreviation, CultureInfo.InvariantCulture));
    }
    ...
    public void GetUnitFromAbbreviation_WithValidQuantityNameAndAbbreviation_ReturnsTheExpectedUnitInfo()
    {
        ...
        Assert.Equal(Area.Info[AreaUnit.SquareMeter], unitInfo);
    }

    [Fact]
    public void GetUnitFromAbbreviation_WithValidQuantityTypeAndAbbreviation_ReturnsTheExpectedUnitInfo()
    {
    ...
    [Fact]
    public void GetUnitFromAbbreviation_WithNullQuantityNameOrAbbreviation_Throws_ArgumentNullException()
    {
        Assert.Throws<ArgumentNullException>(() => UnitParser.Default.GetUnitFromAbbreviation((string)null!, "m²", CultureInfo.InvariantCulture));
        Assert.Throws<ArgumentNullException>(() => UnitParser.Default.GetUnitFromAbbreviation("name", null!, CultureInfo.InvariantCulture));
    }

    [Fact]
    public void GetUnitFromAbbreviation_WithNullQuantityTypeOrAbbreviation_Throws_ArgumentNullException()
    {
        Assert.Throws<ArgumentNullException>(() => UnitParser.Default.GetUnitFromAbbreviation((Type)null!, "m²", CultureInfo.InvariantCulture));
        Assert.Throws<ArgumentNullException>(() => UnitParser.Default.GetUnitFromAbbreviation(typeof(string), null!, CultureInfo.InvariantCulture));
    }

    [Fact]
    public void GetUnitFromAbbreviation_WithInvalidQuantityName_Throws_QuantityNotFoundException()
    {
        Assert.Throws<QuantityNotFoundException>(() => UnitParser.Default.GetUnitFromAbbreviation("InvalidQuantity", "m²", CultureInfo.InvariantCulture));
    }
    ...
    public void GetUnitFromAbbreviation_WithInvalidQuantityType_Throws_ArgumentException()
    {
        ArgumentException ex = Assert.Throws<ArgumentException>(() =>
            UnitParser.Default.GetUnitFromAbbreviation(typeof(string), "m²", CultureInfo.InvariantCulture));
        Assert.Equal($"""Type {typeof(string)} must be of type UnitsNet.IQuantity.""", ex.Message);
    }

    [Fact]
    public void GetUnitFromAbbreviation_WithAmbiguousUnits_ThrowsException()
    {
        AmbiguousUnitParseException ex = Assert.Throws<AmbiguousUnitParseException>(() =>
            UnitParser.Default.GetUnitFromAbbreviation("Length", "pt", CultureInfo.InvariantCulture));
        Assert.Equal("""Cannot parse "pt" since it matches multiple units: DtpPoint ("pt"), PrinterPoint ("pt").""", ex.Message);
    }

    [Fact]
    public void TryGetUnitFromAbbreviation_WithValidQuantityNameAndAbbreviation_ReturnsTrue()
    {
        var success = UnitParser.Default.TryGetUnitFromAbbreviation("Area", "m²", CultureInfo.InvariantCulture, out UnitInfo? unitInfo);
        Assert.True(success);
        Assert.Equal(Area.Info[AreaUnit.SquareMeter], unitInfo);
    }

    [Fact]
    public void TryGetUnitFromAbbreviation_WithValidQuantityTypeAndAbbreviation_ReturnsTrue()
    {
        var success = UnitParser.Default.TryGetUnitFromAbbreviation(typeof(Area), "m²", CultureInfo.InvariantCulture, out UnitInfo? unitInfo);
        Assert.True(success);
        Assert.Equal(Area.Info[AreaUnit.SquareMeter], unitInfo);
    }

    [Fact]
    public void TryGetUnitFromAbbreviation_WithInvalidQuantityName_ReturnsFalse()
    {
    ...
    [Fact]
    public void TryGetUnitFromAbbreviation_WithInvalidQuantityType_ReturnsFalse()
    {
        var success = UnitParser.Default.TryGetUnitFromAbbreviation(typeof(string), "m²", CultureInfo.InvariantCulture, out UnitInfo? unitInfo);
        Assert.False(success);
        Assert.Null(unitInfo);
    }

    [Fact]
    public void TryGetUnitFromAbbreviation_WithAmbiguousUnits_ReturnsFalse()
    {
        var success = UnitParser.Default.TryGetUnitFromAbbreviation("Length", "pt", CultureInfo.InvariantCulture, out UnitInfo? unitInfo);
        Assert.False(success);
        Assert.Null(unitInfo);
    }
}
