// Licensed under MIT No Attribution, see LICENSE file at the root.
// Copyright 2013 Andreas Gullberg Larsen (andreas.larsen84@gmail.com). Maintained at https://github.com/angularsen/UnitsNet.

#if NET8_0_OR_GREATER

using UnitsNet.Core;
using UnitsNet.Units;

namespace UnitsNet;

public readonly partial struct Length : UnitsNet.Core.ILinearQuantity<Length, LengthUnit>
{
    /// <inheritdoc />
    public static QuantityId QuantityId { get; } = new("UnitsNet.Length");

    /// <inheritdoc />
    public static double Convert(double value, LengthUnit fromUnit, LengthUnit toUnit) =>
        From(value, fromUnit).As(toUnit);

    double UnitsNet.Core.IQuantity<double>.Value => Value;

    static Length UnitsNet.Core.IQuantity<Length, LengthUnit, double>.From(double value, LengthUnit unit) =>
        From(value, unit);

    static Length System.Numerics.IMultiplyOperators<Length, double, Length>.operator *(Length left, double right) =>
        left * (QuantityValue)right;

    static Length System.Numerics.IDivisionOperators<Length, double, Length>.operator /(Length left, double right) =>
        left / (QuantityValue)right;

    static Length System.Numerics.IAdditiveIdentity<Length, Length>.AdditiveIdentity => Zero;
}

public readonly partial struct Mass : UnitsNet.Core.ILinearQuantity<Mass, MassUnit>
{
    /// <inheritdoc />
    public static QuantityId QuantityId { get; } = new("UnitsNet.Mass");

    /// <inheritdoc />
    public static double Convert(double value, MassUnit fromUnit, MassUnit toUnit) =>
        From(value, fromUnit).As(toUnit);

    double UnitsNet.Core.IQuantity<double>.Value => Value;

    static Mass UnitsNet.Core.IQuantity<Mass, MassUnit, double>.From(double value, MassUnit unit) =>
        From(value, unit);

    static Mass System.Numerics.IMultiplyOperators<Mass, double, Mass>.operator *(Mass left, double right) =>
        left * (QuantityValue)right;

    static Mass System.Numerics.IDivisionOperators<Mass, double, Mass>.operator /(Mass left, double right) =>
        left / (QuantityValue)right;

    static Mass System.Numerics.IAdditiveIdentity<Mass, Mass>.AdditiveIdentity => Zero;
}

public readonly partial struct Duration : UnitsNet.Core.ILinearQuantity<Duration, DurationUnit>
{
    /// <inheritdoc />
    public static QuantityId QuantityId { get; } = new("UnitsNet.Duration");

    /// <inheritdoc />
    public static double Convert(double value, DurationUnit fromUnit, DurationUnit toUnit) =>
        From(value, fromUnit).As(toUnit);

    double UnitsNet.Core.IQuantity<double>.Value => Value;

    static Duration UnitsNet.Core.IQuantity<Duration, DurationUnit, double>.From(double value, DurationUnit unit) =>
        From(value, unit);

    static Duration System.Numerics.IMultiplyOperators<Duration, double, Duration>.operator *(Duration left, double right) =>
        left * (QuantityValue)right;

    static Duration System.Numerics.IDivisionOperators<Duration, double, Duration>.operator /(Duration left, double right) =>
        left / (QuantityValue)right;

    static Duration System.Numerics.IAdditiveIdentity<Duration, Duration>.AdditiveIdentity => Zero;
}

public readonly partial struct Area : UnitsNet.Core.ILinearQuantity<Area, AreaUnit>
{
    /// <inheritdoc />
    public static QuantityId QuantityId { get; } = new("UnitsNet.Area");

    /// <inheritdoc />
    public static double Convert(double value, AreaUnit fromUnit, AreaUnit toUnit) =>
        From(value, fromUnit).As(toUnit);

    double UnitsNet.Core.IQuantity<double>.Value => Value;

    static Area UnitsNet.Core.IQuantity<Area, AreaUnit, double>.From(double value, AreaUnit unit) =>
        From(value, unit);

    static Area System.Numerics.IMultiplyOperators<Area, double, Area>.operator *(Area left, double right) =>
        left * (QuantityValue)right;

    static Area System.Numerics.IDivisionOperators<Area, double, Area>.operator /(Area left, double right) =>
        left / (QuantityValue)right;

    static Area System.Numerics.IAdditiveIdentity<Area, Area>.AdditiveIdentity => Zero;
}

public readonly partial struct Speed : UnitsNet.Core.ILinearQuantity<Speed, SpeedUnit>
{
    /// <inheritdoc />
    public static QuantityId QuantityId { get; } = new("UnitsNet.Speed");

    /// <inheritdoc />
    public static double Convert(double value, SpeedUnit fromUnit, SpeedUnit toUnit) =>
        From(value, fromUnit).As(toUnit);

    double UnitsNet.Core.IQuantity<double>.Value => Value;

    static Speed UnitsNet.Core.IQuantity<Speed, SpeedUnit, double>.From(double value, SpeedUnit unit) =>
        From(value, unit);

    static Speed System.Numerics.IMultiplyOperators<Speed, double, Speed>.operator *(Speed left, double right) =>
        left * (QuantityValue)right;

    static Speed System.Numerics.IDivisionOperators<Speed, double, Speed>.operator /(Speed left, double right) =>
        left / (QuantityValue)right;

    static Speed System.Numerics.IAdditiveIdentity<Speed, Speed>.AdditiveIdentity => Zero;
}

public readonly partial struct Acceleration : UnitsNet.Core.ILinearQuantity<Acceleration, AccelerationUnit>
{
    /// <inheritdoc />
    public static QuantityId QuantityId { get; } = new("UnitsNet.Acceleration");

    /// <inheritdoc />
    public static double Convert(double value, AccelerationUnit fromUnit, AccelerationUnit toUnit) =>
        From(value, fromUnit).As(toUnit);

    double UnitsNet.Core.IQuantity<double>.Value => Value;

    static Acceleration UnitsNet.Core.IQuantity<Acceleration, AccelerationUnit, double>.From(double value, AccelerationUnit unit) =>
        From(value, unit);

    static Acceleration System.Numerics.IMultiplyOperators<Acceleration, double, Acceleration>.operator *(Acceleration left, double right) =>
        left * (QuantityValue)right;

    static Acceleration System.Numerics.IDivisionOperators<Acceleration, double, Acceleration>.operator /(Acceleration left, double right) =>
        left / (QuantityValue)right;

    static Acceleration System.Numerics.IAdditiveIdentity<Acceleration, Acceleration>.AdditiveIdentity => Zero;
}

public readonly partial struct Force : UnitsNet.Core.ILinearQuantity<Force, ForceUnit>
{
    /// <inheritdoc />
    public static QuantityId QuantityId { get; } = new("UnitsNet.Force");

    /// <inheritdoc />
    public static double Convert(double value, ForceUnit fromUnit, ForceUnit toUnit) =>
        From(value, fromUnit).As(toUnit);

    double UnitsNet.Core.IQuantity<double>.Value => Value;

    static Force UnitsNet.Core.IQuantity<Force, ForceUnit, double>.From(double value, ForceUnit unit) =>
        From(value, unit);

    static Force System.Numerics.IMultiplyOperators<Force, double, Force>.operator *(Force left, double right) =>
        left * (QuantityValue)right;

    static Force System.Numerics.IDivisionOperators<Force, double, Force>.operator /(Force left, double right) =>
        left / (QuantityValue)right;

    static Force System.Numerics.IAdditiveIdentity<Force, Force>.AdditiveIdentity => Zero;
}

public readonly partial struct Pressure : UnitsNet.Core.ILinearQuantity<Pressure, PressureUnit>
{
    /// <inheritdoc />
    public static QuantityId QuantityId { get; } = new("UnitsNet.Pressure");

    /// <inheritdoc />
    public static double Convert(double value, PressureUnit fromUnit, PressureUnit toUnit) =>
        From(value, fromUnit).As(toUnit);

    double UnitsNet.Core.IQuantity<double>.Value => Value;

    static Pressure UnitsNet.Core.IQuantity<Pressure, PressureUnit, double>.From(double value, PressureUnit unit) =>
        From(value, unit);

    static Pressure System.Numerics.IMultiplyOperators<Pressure, double, Pressure>.operator *(Pressure left, double right) =>
        left * (QuantityValue)right;

    static Pressure System.Numerics.IDivisionOperators<Pressure, double, Pressure>.operator /(Pressure left, double right) =>
        left / (QuantityValue)right;

    static Pressure System.Numerics.IAdditiveIdentity<Pressure, Pressure>.AdditiveIdentity => Zero;
}

public readonly partial struct Energy : UnitsNet.Core.ILinearQuantity<Energy, EnergyUnit>
{
    /// <inheritdoc />
    public static QuantityId QuantityId { get; } = new("UnitsNet.Energy");

    /// <inheritdoc />
    public static double Convert(double value, EnergyUnit fromUnit, EnergyUnit toUnit) =>
        From(value, fromUnit).As(toUnit);

    double UnitsNet.Core.IQuantity<double>.Value => Value;

    static Energy UnitsNet.Core.IQuantity<Energy, EnergyUnit, double>.From(double value, EnergyUnit unit) =>
        From(value, unit);

    static Energy System.Numerics.IMultiplyOperators<Energy, double, Energy>.operator *(Energy left, double right) =>
        left * (QuantityValue)right;

    static Energy System.Numerics.IDivisionOperators<Energy, double, Energy>.operator /(Energy left, double right) =>
        left / (QuantityValue)right;

    static Energy System.Numerics.IAdditiveIdentity<Energy, Energy>.AdditiveIdentity => Zero;
}

public readonly partial struct Power : UnitsNet.Core.ILinearQuantity<Power, PowerUnit>
{
    /// <inheritdoc />
    public static QuantityId QuantityId { get; } = new("UnitsNet.Power");

    /// <inheritdoc />
    public static double Convert(double value, PowerUnit fromUnit, PowerUnit toUnit) =>
        From(value, fromUnit).As(toUnit);

    double UnitsNet.Core.IQuantity<double>.Value => Value;

    static Power UnitsNet.Core.IQuantity<Power, PowerUnit, double>.From(double value, PowerUnit unit) =>
        From(value, unit);

    static Power System.Numerics.IMultiplyOperators<Power, double, Power>.operator *(Power left, double right) =>
        left * (QuantityValue)right;

    static Power System.Numerics.IDivisionOperators<Power, double, Power>.operator /(Power left, double right) =>
        left / (QuantityValue)right;

    static Power System.Numerics.IAdditiveIdentity<Power, Power>.AdditiveIdentity => Zero;
}

public readonly partial struct Temperature : UnitsNet.Core.IAffineQuantity<Temperature, TemperatureUnit, TemperatureDelta>
{
    /// <inheritdoc />
    public static QuantityId QuantityId { get; } = new("UnitsNet.Temperature");

    /// <inheritdoc />
    public static double Convert(double value, TemperatureUnit fromUnit, TemperatureUnit toUnit) =>
        From(value, fromUnit).As(toUnit);

    double UnitsNet.Core.IQuantity<double>.Value => Value;

    static Temperature UnitsNet.Core.IQuantity<Temperature, TemperatureUnit, double>.From(double value, TemperatureUnit unit) =>
        From(value, unit);
}

public readonly partial struct TemperatureDelta : UnitsNet.Core.ILinearQuantity<TemperatureDelta, TemperatureDeltaUnit>
{
    /// <inheritdoc />
    public static QuantityId QuantityId { get; } = new("UnitsNet.TemperatureDelta");

    /// <inheritdoc />
    public static double Convert(double value, TemperatureDeltaUnit fromUnit, TemperatureDeltaUnit toUnit) =>
        From(value, fromUnit).As(toUnit);

    double UnitsNet.Core.IQuantity<double>.Value => Value;

    static TemperatureDelta UnitsNet.Core.IQuantity<TemperatureDelta, TemperatureDeltaUnit, double>.From(double value, TemperatureDeltaUnit unit) =>
        From(value, unit);

    static TemperatureDelta System.Numerics.IMultiplyOperators<TemperatureDelta, double, TemperatureDelta>.operator *(TemperatureDelta left, double right) =>
        left * (QuantityValue)right;

    static TemperatureDelta System.Numerics.IDivisionOperators<TemperatureDelta, double, TemperatureDelta>.operator /(TemperatureDelta left, double right) =>
        left / (QuantityValue)right;

    static TemperatureDelta System.Numerics.IAdditiveIdentity<TemperatureDelta, TemperatureDelta>.AdditiveIdentity => Zero;
}

public readonly partial struct Level : UnitsNet.Core.ILogarithmicQuantity<Level, LevelUnit>
{
    /// <inheritdoc />
    public static QuantityId QuantityId { get; } = new("UnitsNet.Level");

    /// <inheritdoc />
    public static double Convert(double value, LevelUnit fromUnit, LevelUnit toUnit) =>
        From(value, fromUnit).As(toUnit);

    double UnitsNet.Core.IQuantity<double>.Value => Value;

    static Level UnitsNet.Core.IQuantity<Level, LevelUnit, double>.From(double value, LevelUnit unit) =>
        From(value, unit);

    static double UnitsNet.Core.ILogarithmicQuantity<Level>.LogarithmicScalingFactor => LogarithmicScalingFactor;
}

public readonly partial struct Information : UnitsNet.Core.ILinearQuantity<Information, InformationUnit>
{
    /// <inheritdoc />
    public static QuantityId QuantityId { get; } = new("UnitsNet.Information");

    /// <inheritdoc />
    public static double Convert(double value, InformationUnit fromUnit, InformationUnit toUnit) =>
        From(value, fromUnit).As(toUnit);

    double UnitsNet.Core.IQuantity<double>.Value => Value;

    static Information UnitsNet.Core.IQuantity<Information, InformationUnit, double>.From(double value, InformationUnit unit) =>
        From(value, unit);

    static Information System.Numerics.IMultiplyOperators<Information, double, Information>.operator *(Information left, double right) =>
        left * (QuantityValue)right;

    static Information System.Numerics.IDivisionOperators<Information, double, Information>.operator /(Information left, double right) =>
        left / (QuantityValue)right;

    static Information System.Numerics.IAdditiveIdentity<Information, Information>.AdditiveIdentity => Zero;
}

#endif
