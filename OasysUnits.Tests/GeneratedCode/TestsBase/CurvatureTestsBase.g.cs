...
// ReSharper restore VirtualMemberNeverOverriden.Global
        ...
        {
            Assert.Throws<ArgumentException>(() => new Curvature((double)0.0, CurvatureUnit.Undefined));
        }
        ...
        public void Curvature_QuantityInfo_ReturnsQuantityInfoDescribingQuantity()
        {
            ...
            Assert.Equal(QuantityType.Curvature, quantityInfo.QuantityType);

            var units = EnumUtils.GetEnumValues<CurvatureUnit>().Except(new[] {CurvatureUnit.Undefined}).OrderBy(x => x.ToString()).ToArray();
        ...
        public void ToUnit_FromNonBaseUnit_ReturnsQuantityWithGivenUnit(CurvatureUnit unit)
        {
            ...
            var fromUnit = Curvature.Units.FirstOrDefault(u => u != Curvature.BaseUnit && u != CurvatureUnit.Undefined);

            // If there is only one unit for the quantity, we must use the base unit.
            if (fromUnit == CurvatureUnit.Undefined)
                fromUnit = Curvature.BaseUnit;
        ...
        {
            var a = Curvature.FromPerMeters(1);
            var b = Curvature.FromPerMeters(2);
...
#pragma warning restore CS8073
        ...
        {
            object a = Curvature.FromPerMeters(1);
            object b = Curvature.FromPerMeters(2);
        ...
        {
            Assert.DoesNotContain(CurvatureUnit.Undefined, Curvature.Units);
        }
        ...
        {
            var quantity = Curvature.FromPerMeters(1.0);
            Assert.Equal(QuantityType.Curvature, Convert.ChangeType(quantity, typeof(QuantityType)));
        }
