...
namespace UnitsNet.Tests
{
    public class QuantityTest
    {
        ...
        {
            Assert.False(Quantity.TryFrom(value, LengthUnit.Centimeter, out var parsedLength));
        ...
        public void GetInfo_GivenLength_ReturnsQuantityInfoForLength()
        {
            ...
            var quantityInfo = Quantity.GetInfo(QuantityType.Length);
...
#pragma warning restore 618
        ...
        public void GetInfo_GivenMass_ReturnsQuantityInfoForMass()
        {
            ...
            var quantityInfo = Quantity.GetInfo(QuantityType.Mass);
...
#pragma warning restore 618
        ...
        {
            var knownQuantityInfos = new[] {Quantity.GetInfo(QuantityType.Length), Quantity.GetInfo(QuantityType.Force), Quantity.GetInfo(QuantityType.Mass)};
        ...
        {
            Assert.True(Quantity.TryFrom(3, LengthUnit.Centimeter, out var parsedLength));
            Assert.Equal(Length.FromCentimeters(3), parsedLength);

            Assert.True(Quantity.TryFrom(3, MassUnit.Tonne, out var parsedMass));
            Assert.Equal(Mass.FromTonnes(3), parsedMass);

            Assert.True(Quantity.TryFrom(3, PressureUnit.Megabar, out var parsedPressure));
        ...
        {
            Assert.False(Quantity.TryParse(typeof(DummyIQuantity), "3.0 cm", out var parsedLength));
            Assert.Null(parsedLength);
        }
        ...
        {
            Assert.False(Quantity.TryParse(typeof(Length), "x cm", out var parsedLength));
            Assert.Null(parsedLength);

            Assert.False(Quantity.TryParse(typeof(Mass), "xt", out var parsedMass));
            Assert.Null(parsedMass);

            Assert.False(Quantity.TryParse(typeof(Pressure), "foo", out var parsedPressure));
        ...
        {
            Assert.True(Quantity.TryParse(typeof(Length), "3 cm", out var parsedLength));
            Assert.Equal(Length.FromCentimeters(3), parsedLength);

            Assert.True(Quantity.TryParse(typeof(Mass), "03t", out var parsedMass));
            Assert.Equal(Mass.FromTonnes(3), parsedMass);

            Assert.True(Quantity.TryParse(NumberFormatInfo.InvariantInfo, typeof(Pressure), "3.0 Mbar", out var parsedPressure));
            Assert.Equal(Pressure.FromMegabars(3), parsedPressure);
        }
