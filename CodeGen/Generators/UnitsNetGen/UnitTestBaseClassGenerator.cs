...
");
        ...
        {{
            var inBaseUnit = {_quantity.Name}.From(1.0, {_quantity.Name}.BaseUnit);
        ...
        {{
            var inBaseUnit = {_quantity.Name}.From(1.0, {_quantity.Name}.BaseUnit);
            Assert.Throws<NotSupportedException>(() => inBaseUnit.ToUnit(default({_unitEnumName})));
        }}
...
");
        ...
        {{
            var quantity = {_quantity.Name}.From{_baseUnit.PluralName}(1.0);
            Assert.Equal({_quantity.Name}.Info.Name.GetHashCode(), quantity.GetHashCode());
        }}
");
